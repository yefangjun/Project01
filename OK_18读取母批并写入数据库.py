...
def decrypt(key, cipher):
    ...
    for i in range(0, len(cipher)):
        fun_return += chr(ord(cipher[i]) - keyList[i % len(keyList)])
        # print(chr(ord(cipher[i]) - keyList[i % len(keyList)]), end='')
    return fun_return
...
if not os.path.exists(finishedPath):
    ...
    print("finishedPath文件夹已新增")
print("finishedPath文件夹已存在")
if not os.path.exists(DefectStaticPath):
    os.mkdir(DefectStaticPath)
    print("DefectStaticPath文件夹已新增")
print("DefectStaticPath文件夹已存在")
