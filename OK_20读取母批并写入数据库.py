...
print("start query etching_info table processing to match……")
...
cur.close()  # 关闭游标
conn.close()  # 关闭数据库连接
print("etching_info table query has finished，get data values：\n%s"%datafm.head())
print("Start Circular MasterBatch folder")
for file in files:
    # 判断文件是不是csv文件
    if file.split('.')[-1] in ['csv']:
        ...
        print("Start read MasterBatch folder：%s"%file)
        ...
        # config.write(fh)
        # fh.close()
        print("Start import data to database table…………")
        ...
        if np_FileMupi[maxRow][0] == 'over':
            ...
            print("MasterBatch table already Over，already move off")
        os.remove(DefectStaticCopy)
        print("The copied AOIADI original table has been deleted")

end = time.time()
print('run time in total: %s second' % (end - start))
output_file.close()